...
// Целочисленное хеширование для генерации псевдослучайных градиентов
...
procedure RandomGradient2D(ix, iy: Integer; out gx, gy: Double);
var
  h: Cardinal;
  a: Double;
begin
  h := Hash2D(ix, iy);
  a := (h / 4294967295.0) * 2 * Pi;
  gx := Cos(a);
  gy := Sin(a);
end;

procedure RandomGradient3D(ix, iy, iz: Integer; out gx, gy, gz: Double);
var
  h1, h2: Cardinal;
  theta, phi: Double;
begin
  h1 := Hash3D(ix, iy, iz);
  h2 := Hash3D(iy, iz, ix); // Смещаем оси для независимого хеша
  theta := (h1 / 4294967295.0) * 2 * Pi;
  phi := ArcCos(1.0 - 2.0 * (h2 / 4294967295.0));
  gx := Sin(phi) * Cos(theta);
