...
begin Result := FIni.ReadInteger('Display', 'ViewWidth', 320); end;

function TConfig.GetViewHeight: Integer;
begin Result := FIni.ReadInteger('Display', 'ViewHeight', 200); end;

function TConfig.GetFullScreen: Boolean;
begin Result := FIni.ReadBool('Display', 'FullScreen', True); end;

function TConfig.GetFontFileName: string;
begin Result := FIni.ReadString('Display', 'FontFile', 'GothicRus.ttf'); end;

function TConfig.GetWorldSize: Double;
begin Result := FIni.ReadFloat('World', 'Size', 400.0); end;

function TConfig.GetTerrainPoints: Integer;
begin Result := FIni.ReadInteger('World', 'TerrainPoints', 1500); end;

function TConfig.GetCloudCount: Integer;
begin Result := FIni.ReadInteger('World', 'CloudCount', 30); end;

function TConfig.GetGravity: Double;
begin Result := FIni.ReadFloat('Player', 'Gravity', 600.0); end;

function TConfig.GetJumpSpeed: Double;
begin Result := FIni.ReadFloat('Player', 'JumpSpeed', 350.0); end;
...
begin Result := FIni.ReadFloat('Player', 'StartX', 0.0); end;

function TConfig.GetStartY: Double;
begin Result := FIni.ReadFloat('Player', 'StartY', 0.0); end;

function TConfig.GetStartZ: Double;
begin Result := FIni.ReadFloat('Player', 'StartZ', 0.0); end;

function TConfig.GetTimerInterval: Integer;
begin Result := FIni.ReadInteger('Timer', 'Interval', 30); end;
