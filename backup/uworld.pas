...
interface
uses
Classes, SysUtils, Math, Contnrs, uTypes, uConfig, uNoise, Generics.Collections;
const
WATER_LEVEL = 24;
BASE_HEIGHT = 32;
HEIGHT_VARIATION = 16;
...
var CX, CZ, lx, lz: Integer; Chunk: TChunk;
begin
if (Y < 0) or (Y >= CHUNK_SIZE_Y) then Exit(0);
CX := GetChunkCoord(X, CHUNK_SIZE_X);
CZ := GetChunkCoord(Z, CHUNK_SIZE_Z);
lx := X - CX * CHUNK_SIZE_X; lz := Z - CZ * CHUNK_SIZE_Z;
Chunk := FindChunk(CX, CZ);
if Chunk = nil then begin EnsureChunkExists(CX, CZ); Chunk := FindChunk(CX, CZ); end;
if Assigned(Chunk) then Result := Chunk.Blocks[lx, Y, lz] else Result := 0;
end;
...
if (Value <> Ord(btAir)) and (Chunk.HeightMap[lx, lz] < Y) then Chunk.HeightMap[lx, lz] := Y;
end;
end;
